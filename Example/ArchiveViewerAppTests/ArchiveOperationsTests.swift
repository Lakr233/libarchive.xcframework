...
final class ArchiveOperationsTests: XCTestCase {
    ...
    func testPlainZipRoundTripListExtractPreview() throws {
        ...
        let one = folder.appendingPathComponent("only-alpha.txt")
        try ArchiveOperations.extract(archive: archive, entry: "alpha.txt", to: one)
        XCTAssertEqual(try String(contentsOf: one, encoding: .utf8), "alpha contents\n")
        XCTAssertFalse(FileManager.default.fileExists(atPath: folder.appendingPathComponent("only-nested").path))
    ...
    func testExtractOneReplacesExistingFile() throws {
        ...
        let dest = folder.appendingPathComponent("out.txt")
        try "much longer leftover payload that must not remain\n".write(to: dest, atomically: true, encoding: .utf8)
        try ArchiveOperations.extract(archive: archive, entry: "alpha.txt", to: dest)
